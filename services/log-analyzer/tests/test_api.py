...
def test_analyze_log_success(monkeypatch):
    mock_generate = MagicMock(return_value=LogAnalysis(
        severity="MEDIUM",
    ...
    response = client.post("/analyze-log", json={"raw_log": "java.net.SocketTimeoutException: Read timed out"})
...
def test_analyze_log_upstream_timeout(monkeypatch):
    ...
    response = client.post("/analyze-log", json={"raw_log": "Any log will trigger the mocked timeout here ..."})
