...
| [`services/gateway`](./services/gateway) | **Live at `aiops.crypticani.dev`.** Single FastAPI entrypoint tying the four services above into one AI DevOps copilot. `POST /ask` takes a plain-English question and an LLM routes it to whichever service can answer, with the reply attributed to it — and two distinct ways to refuse: the model can say it cannot place the question, and the gateway can say the right service was found but the request lacks the data it needs. Aggregated `/health` reports every backend's own health, not just its own liveness | ✅ Complete |
...
* [X] **Day 26: Runtime signals through the same pipeline** — Kubernetes audit events (`kubectl exec`, Secret reads, RBAC writes, anything the API server refused) normalised into the same `Finding`, POSTed to the same `/triage`, scored by the same `risk.py` and rendered by the same `comment.py`. The cost of accepting a fourth kind of input was one parser and one line in `_PARSERS` — nothing downstream was modified, because nothing downstream ever needed a file or a line or a package, only "a thing worth a judgment, and where it is". `runtime.py` is the collector, `scan.sh`'s counterpart on the cluster side; `kind/audit-policy.yaml` bounds *volume* and the rule table in `scanners.py` bounds *meaning*, so an audited event matching no rule is not a finding. The policy logs at `Metadata` level and never `RequestResponse`, because the request body of a Secret write and the response body of a Secret read are both the secret — an audit log that records what it is auditing is a liability, not a control. Verified end to end on a second `kind` cluster: 5 events → 4 findings → score 21, `pass`, with an RBAC grant rated `high` and a pod `exec` rated `low`.
* **The finding:** three, and the interesting one is about identity. A runtime finding's identity genuinely includes *who did it*, so the actor is spelled into `target` (`kubernetes-admin@sandbox/demo-creds`) rather than kept beside it — `_fingerprint` reads `target` for a finding with no package and no line, so any other arrangement gives two different users reading the same Secret one shared fingerprint and `dedupe` correctly drops one of them. The round trip then found a bug 111 green tests could not: a `create` carries no name in its `objectRef` (the name is in the request body, which `Metadata` level deliberately omits) and neither does a `list`, so two unrelated refusals collapsed into one target — and the test that existed asserted that behaviour as correct, because it had been written from the implementation. And the first real capture was **19 of 22 findings being the cluster installing itself**, which has no code fix worth writing: `kubeadm` and `kubectl` authenticate as the same identity and the only field separating them is `userAgent`, so filtering on it would mean anyone who sets `User-Agent: kubeadm` disappears from triage.

* [X] **Day 27: What a run costs** — token accounting through all four services' provider seams, and `bench.py` sweeping `ST_BATCH_SIZE` across 1/3/5/10. Cost is reported in tokens and *requests* per 1,000 findings rather than dollars: both backends are free at the margin, and what a free-tier quota actually charges is calls, which is exactly what batch size buys down. A curve fitted to three single calls — `prompt = 368 + 70.5n`, `output = 14 + 78.5n` — predicted a nine-call, 43-finding corpus run to within **0.13% and 0.85%**, so a whole-corpus figure never has to be sat through. `ST_BATCH_SIZE` stays 5, and the honest reason is that it is *not* the cheapest option — batch 10 uses 14% fewer tokens and half the requests. What decides it is margin: batch 10 runs 339.7s against a 600s ceiling, and the same config demonstrably varied 2x on a loaded laptop (158.7s → 208.4s → over 300s), so the cheaper setting is only cheaper when it completes. `ST_MAX_TOKENS=1536` was the expected ceiling and is the wrong one — it doesn't bind until ~19 findings, while wall clock is output-bound at 1.7–3.2 tok/s, so the *timeout* binds first. That is why `LLM_TIMEOUT` became this service's own `ST_LLM_TIMEOUT` at 600s, and batch 10 completing at all is that change paying for itself.
* **The findings:** three, and none of them were batch size. **94% of the corpus should never have existed** — running the same scan both ways on the same afternoon, 791 of 841 deduped findings were `bandit:B101` "use of assert detected" inside test files, because `scan.sh` never excluded them. An assert is what a test file is made of. One line took the corpus to 44, model calls from 168 to 9, and a full run from ~6 hours to ~20 minutes; the model also *declines* B101, so a full-corpus run would have marked ~94% `needs_human` and routed 791 non-issues to a human, with every guard green throughout. The cheapest model call is the one you don't make. Second, **field order in a grammar-constrained schema is behaviour**: `priority` was declared first in `TriageResult`, so it was generated before `exploitability` and `impact` existed, making the prompt's "weighing both of the above" impossible to obey — and it came out anti-correlated with its own inputs. Reordering to dependency order eliminated explanation truncation (160 → 89 chars) and made explanations cite the ratings above them, though priority *calibration* is still wrong and is left measured rather than tuned against 15 unlabelled findings. The same bug, found the same day, in `log-analyzer`'s `LogAnalysis`. Third, **two silent-failure bugs found only by running things**: `scan.sh` swallowed `bandit: command not found` and wrote a trivy-only envelope that read exactly like a clean scan, and log-analyzer had been sending `temperature` at the payload's top level, where Ollama ignores it — so every call ran at 0.8 and its golden-set eval had always been a coin flip with a report attached.

* [X] **Day 28: Capstone — metrics, evals, and the deploy** — `metrics.py` and `/metrics` with **`repo` as a label**, which none of the other three services needed: this is the only multi-tenant thing here, so "how many runs" and "how much risk" are the wrong questions without a "whose". `eval_triage.py` + `eval_set.json` is the gate the test suite cannot be — 153 unit tests can be green while the model has quietly got worse, because no unit test in this repo ever calls a model, and Day 27's schema-order bug is the worked example of exactly that. And `eval_all.py` at the repo root runs **every** service's eval and prints one table, which meant building a golden set for the self-healing agent too: four alerts against a stubbed cluster and a real model, grading the one output a human can click. Then the deploy — `Dockerfile`, multi-arch GHCR publish, a compose entry behind a loopback bind, an eleven-panel Grafana dashboard whose every expression is checked against `metrics.py` by a test (an empty graph is indistinguishable from a quiet service), and `k8s/` manifests that pass this service's own gate (their own namespace, `securityContext` at both levels, `readOnlyRootFilesystem`), because shipping a fifth manifest set carrying findings this service exists to triage would be the project arguing with itself.
* **Deployed and dogfooded:** live at its own subdomain behind Cloudflare and nginx, scraped by Prometheus with an 11-panel Grafana dashboard, and gating pull requests on this very repo — 131 findings from the scanners, 58 after dedup, a comment with rule ids, paths, a verdict and three proposed diffs. Two faults surfaced only once the endpoint was real: `gh pr comment` needs `-R` because the reporting job deliberately never checks the caller out, and the gate found two findings on the manifests written for it the same day (one fixed, one a documented refusal).
* **And the eval did its job on the first run, by failing.** One in-band judgment out of twelve, both at batch 5 and at batch 1 — and the two runs separate two distinct defects. The *declines* are a missing-input problem: `_format_finding` never sends the context lines `scanners.py` captures, and the model said *"insufficient context to judge exploitability and impact"* five times unprompted. The *over-calling* is a missing-rubric problem: nothing defines what `critical` means for a scanner finding, the same gap Day 1 found in log-analyzer. Both are measured, named and deferred rather than patched at the end of a long day — sending context triples per-finding prompt tokens, and the batch-1 run showed prompt eval is what dominates wall clock, so it needs a before/after rather than a prompt edit.
...
* **The eval, four runs — 18/24, 13/24, 20/24, 22/24.** `qwen2.5:7b-instruct` ships, measured rather than assumed: 18/24 at 10.2s per route against `qwen2.5-coder:1.5b`'s 13/24 at 6.3s, same token cost. One variable per run after that, so each step is attributable. **I wrote the router's worst bias myself.** Three of the six run-1 misses went to knowledge-copilot, whose catalogue entry opened *"Answers operational questions"* — the broadest phrase in the set, in the entry for the one backend needing no attachment, closing with *"Needs nothing but the question."* The model's stated reason for the worst miss was the two-word quotation `operational question`. Narrowing that one description took knowledge-copilot from **8 picks / 3 wrong → 5 picks / 0 wrong**, and it stayed fixed through the next change. Caveat on record: the two vague misses swapped identity between runs 1 and 2, so ±1–2 is noise and each +2 sits at the edge of what one run can claim.
* **The correction: grammar-constrained decoding enforces structure, not value ranges.** This service's headline claim was *"the schema is the guard, not the validator"*, and that was true of the half I designed and false of the half I assumed came free. `service` never once left its enum in 72 calls — an enum is structural and the sampler cannot emit a token outside it. `ge`/`le` on a float are Pydantic rejecting a value *after* generation, so `"confidence": 10` from the 1.5b (×4) and `"confidence": 2` from the **7b that ships** were each a 502. Two constraints in the same `Field(...)` call, one load-bearing. My first instinct was to keep the 502 as an honest signal about a bad model; the next run showed that was wrong, because it was really ~4% of production `/ask` calls failing for a non-routing reason. `confidence` is now `Literal["low","medium","high"]` — structural, like `service` — and the floor became `GW_ROUTE_ON=medium`.
* **And the result I didn't expect: the shape of a schema field changed the model's behaviour, not just its legal values.** As a float, `confidence` came back `1.00` or `0.00` across 48 graded routes and *nothing in between* — every `0.00` attached to `service: "none"`, so the floor changed no outcome at all, zero times. A boolean wearing a float's clothing, exactly what the `gw_router_confidence` series was added to expose. As `low|medium|high`, the same model on the same 24 questions used **all three** (15/2/7). Nothing had stopped it returning `0.6` before. Asking for a number got a number treated as a switch; asking for one of three named judgements got a judgement. The same change appears to have fixed something it wasn't aimed at: `reason` is generated before `service` so the pick is conditioned on stated reasoning, and it had been naming the *destination* instead — the two halves of the discrimination pair came back with **byte-identical reasons** (`log analysis required`) and opposite correctness. After the enum they describe the question (`asked for meaning of a specific log entry` vs `the question asks for remediation`) and the pair separated. Two honest limits: it was one decision but two edits (the field *and* its prompt sentence), and it is one run.
...
* [X] ~~Measure the gateway's router and set `GW_OLLAMA_MODEL` from the result.~~ Done: 18/24 vs 13/24, `qwen2.5:7b-instruct` ships.
* [X] ~~Re-run the router eval after narrowing the copilot's description.~~ Done: 18/24 → 20/24, knowledge-copilot 3 wrong → 0.
* [X] ~~Decide whether `confidence` earns its place.~~ Done: it became the `low|medium|high` enum, because the shipped model emitted an out-of-range float too.
* [ ] **Make `POST /ask` return `202` and a poll URL.** The clearest design error in the gateway: it answers inline, copying knowledge-copilot's shape, but its latency profile is security-triage's — 9s warm, 190s cold. Triage already returns `202` + poll for precisely that reason and I copied from the wrong sibling. It also removes a hard ceiling: Cloudflare fronts `aiops.crypticani.dev` and returns `error code: 524` at 125s regardless of what nginx and the gateway allow.
* [ ] **Give the router a model that fits the host.** `free -h` on the box hosting Ollama: 764Mi free, 2.7Gi already swapped, and `ollama ps` empty right after a successful call *with* `OLLAMA_KEEP_ALIVE` set — a 4.7GB model can load there and cannot stay. Test `qwen2.5:3b` (~1.9GB) with `eval_router.py --model`; the 13/24 that disqualified a small model was `qwen2.5-coder:1.5b`, a *coder* tune doing prose classification, which is the one thing the router chose an instruct tune to avoid.
* [ ] **A Gemini reachability check in `/health`, if the router ever moves to Gemini.** The Ollama branch checks `/api/tags`; there is no equivalent, so an exhausted free-tier quota would read as `healthy` while every `/ask` 502s. And `GW_MAX_ASKS_PER_HOUR=60` permits 1,440 calls/day against a ~1,000/day cap, so the gateway's own limit would not protect the quota.
* [ ] **Fix the descriptions that argue with the prompt.** Now the top item, because it is the only remaining miss with a diagnosis. The rules say *"whether the caller supplied the data a service needs is not your problem"*, and then three catalogue entries end with *"so the log must be supplied"* / *"a description of an alert is not an alert."* That contradiction is the likely cause of the one case that has missed **every** run: a bare *"can something scale it back up?"* gets declined instead of routed to the agent, where `needs_input` would have handled the missing alert correctly. Run 3 showed it reporting `low` rather than flatly declining, which is the router hesitating rather than disagreeing.
* [ ] **Separate the two halves of the confidence change.** It was one design decision and two edits — the schema field and its prompt sentence — so which one produced the behaviour change is unknown. One run with the enum and the old wording answers it.
* [ ] **Then reorder the catalogue.** Position bias is the remaining live hypothesis for knowledge-copilot's pull — it is listed first. One variable at a time so the eval can attribute it, same discipline as the triage pair below.
* [ ] **Confirm whether `reason` needs the prompt change at all.** It was going to be the next fix, and then the enum appeared to fix it as a side effect. A prompt demanding `reason` quote the words that decided the route is now a confirmation rather than a repair — and if the reasons are already question-shaped without it, that is worth knowing too.
