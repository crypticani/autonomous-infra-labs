...
* **The eval, three runs — 18/24, 13/24, then 20/24.** `qwen2.5:7b-instruct` ships, measured rather than assumed: 18/24 at 10.2s per route against `qwen2.5-coder:1.5b`'s 13/24 at 6.3s, same token cost. **I wrote the router's worst bias myself.** Three of the six misses went to knowledge-copilot, whose catalogue entry opened *"Answers operational questions"* — the broadest phrase in the set, in the entry for the one backend needing no attachment, closing with *"Needs nothing but the question."* The model's stated reason for the worst miss was the two-word quotation `operational question`. Narrowing that one description, changing nothing else, took knowledge-copilot from **8 picks / 3 wrong → 5 picks / 0 wrong** and the score to 20/24. Caveat on record: the two vague misses swapped identity between runs, so ±1–2 is noise and 18 → 20 sits at the edge of what one run can claim. The 3 → 0 is the part that isn't ambiguous.
...
* **And a negative result worth keeping:** the confidence floor **changed no outcome at all**, zero times across 48 graded routes. Every 7b confidence was `1.00` or `0.00`, and each `0.00` arrived with `service: "none"`, which declines on the enum branch before the floor is consulted — a boolean wearing a float's clothing, exactly what the `gw_router_confidence` series was added to expose. Related: `reason` is generated before `service` so the pick is conditioned on stated reasoning, and it isn't working — the two halves of the discrimination pair came back with **byte-identical reasons** (`log analysis required`) and opposite correctness. The model is naming its destination, not reading the question.
...
* [ ] **Make `reason` quote the question instead of naming the service.** The strongest finding of the last run and the next single-variable prompt change: the discrimination pair returned byte-identical reasons with opposite correctness, so `reason`-before-`service` is buying nothing as written. Ask it to quote the words that decided the route.
* [ ] **Then reorder the catalogue.** Position bias is the remaining live hypothesis for knowledge-copilot's pull — it is listed first. One variable at a time, so the eval can attribute it, which is the same discipline as the triage pair below.
* [ ] **Fix the descriptions that argue with the prompt.** The rules say *"whether the caller supplied the data a service needs is not your problem"*, and then three catalogue entries end with *"so the log must be supplied"* / *"a description of an alert is not an alert."* That contradiction is the likely cause of the one miss surviving every run: a bare *"can something scale it back up?"* gets declined instead of routed to the agent, where `needs_input` would have handled the missing alert correctly.
