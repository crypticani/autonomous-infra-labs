...
* **The eval, run both ways — 18/24 and 13/24.** `qwen2.5:7b-instruct` ships, measured rather than assumed: 18/24 at 10.2s per route against `qwen2.5-coder:1.5b`'s 13/24 at 6.3s, same token cost. And the 1.5b's denominators are 17 and 3 rather than 19 and 5, because **four of its calls never produced a valid route** — all four the same way, `"confidence": 10` against a schema declaring `le=1.0`. Which corrects this service's own headline claim: **grammar-constrained decoding enforces structure, not value ranges.** `service` never once left its enum in 48 calls across both models, because an enum is structural and the sampler cannot emit a token outside it; `ge`/`le` on a float are Pydantic rejecting a value after the fact, which is a 502 and not a guard. Two constraints in the same `Field(...)` and only one of them load-bearing.
* **Two more, both negative results, both mine:** `GW_MIN_CONFIDENCE=0.6` **changed no outcome at all** — every 7b confidence was `1.00` or `0.00`, and each `0.00` arrived with `service: "none"`, which declines on the enum branch before the floor is consulted. Confidence is a boolean wearing a float's clothing, exactly what the `gw_router_confidence` histogram was added to expose. And three of the six misses went to knowledge-copilot because **I wrote the attractor myself**: its catalogue entry opened *"Answers operational questions"* — the broadest phrase in the set, in the entry for the one backend needing no attachment, closing with *"Needs nothing but the question"* — and the model's stated reason for the worst miss was the two-word quotation `operational question`. Narrowed, one variable changed, ordering deliberately left alone as the next single-variable experiment.
...
* [ ] **Re-run the router eval, then reorder the catalogue if it hasn't moved.** The knowledge-copilot description was narrowed after it accounted for three of six misses; the ordering bias — it is listed first — is the other live hypothesis for the same pull, and is the next thing to change *on its own* so the eval can attribute it. Same discipline as the triage pair below, for the same reason.
* [ ] **Decide whether `confidence` earns its place at all.** It changed no outcome on the shipped model. Either it becomes a structural constraint the grammar can actually enforce — a `low|medium|high` enum, the way `service` already is — or it comes out and the router declines only via `none`. Answering that needs one more run, not a redesign.
